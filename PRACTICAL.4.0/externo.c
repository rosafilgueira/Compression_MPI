...
{
    return 0;
/* Commented because there's only one option - lz4
if (len==4)
  return start_int;
else
  if (len==8)
...
double time_algorithm (void *buf, int total_len, int algorithm)
...
*/
int search_algorithm(void *buf, int len, int total_len)
{
int algorithm,algorithm_result;
double time_rle,time_lzo,time_huff,time_rice, time_snappy, time_lz4,time_orig;
double band, speed[6];
...
*/
int select_algorithm(void *buf, int len, int total_len)
{
    return 14;

/* 
   Commented for now, because there's only one algo option - lz4.
...
int hash(MPI_Request request_position) {
    ...
    return abs(request_position % SIZE);
}
...
//###########################################################
 ...
 //  MPI_Comm_rank(comm,&rank);
   ...
   flag=estudio_cabecera(vSend3);
    if (flag==1)
    { total_len=status->count;  
      descompresion(vSend3,buf,total_len,0);
    }
   else
    memcpy(buf,vSend3,status->count);
    free(vSend3);
  return 0; 
  }
...
int MPI_Wait( MPI_Request  *request, MPI_Status   *status) {
  int flag=0;
  int total_len,position,original_size,rank;
  char *vSend;
   ...
   if(table_request[position].type==1)
    { 
     ...
     if (flag==1)
      ...
      {original_size=status->count;
       memcpy(table_request[position].buf_user,table_request[position].buf_recv,status->count);
      } 
