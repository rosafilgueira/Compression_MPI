...
int MPI_Send(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm){
 ...
 //  MPI_Comm_rank(comm,&rank);
   ...
   flag=estudio_cabecera(vSend3);
    if (flag==1)
    { total_len=status->_ucount;  
      descompresion(vSend3,buf,total_len,0);
    }
   else
    memcpy(buf,vSend3,status->_ucount);
    free(vSend3);
  return 0; 
  }
   ...
   if(table_request[position].type==1)
    { 
     ...
     if (flag==1)
      ...
      {original_size=status->_ucount;
       memcpy(table_request[position].buf_user,table_request[position].buf_recv,status->_ucount);
      } 
